...
class TableViewModel<T> extends AbstractTableModel
{
    ...
    */
	...
	{
		List<ColumnDefinition> columnDefinitions = controller.getVisibleColumns();
	...
	protected void publish(RowPublisher<? super T> publisher)
	{
		for (T data : this.rowData) {
	...
	private void setRange(Object value, int column, int curRow, int prevRow)
	{
		...
		int viewRow1 = this.controller.getInstalledTable().convertRowIndexToView(curRow);
		int viewRow2 = this.controller.getInstalledTable().convertRowIndexToView(prevRow);
		...
		for (int viewIndex = viewFrom; viewIndex <= viewTo; viewIndex++) {
			...
			int modelIndex = this.controller.getInstalledTable().convertRowIndexToModel(viewIndex);
